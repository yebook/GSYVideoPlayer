...
//        mIfCurrentIsFullscreen = fullFlag;
    ...
    protected void startPrepare() {
        ...
        getGSYVideoManager().prepare(mUrl, (mMapHeadData == null) ? new HashMap<String, String>() : mMapHeadData, mLooping, mOverrideExtension);
     ...
     * @return
     */
    public boolean setUp(String url, Map<String, String> mapHeadData, String title) {
        if (setUp(url, title)) {
     ...
     * @return
     */
    protected boolean setUp(String url, String title, boolean changeState) {
        mOriginUrl = url;
    ...
    public void onPrepared() {
        if (gsyMediaBackListener != null) {
            gsyMediaBackListener.onPrepared();
        }
...
//        }
    ...
    public void onCompletion() {
        if (gsyMediaBackListener != null) {
            gsyMediaBackListener.onCompletion();
        }
    ...
    public void onError(int what, int extra) {
        if (gsyMediaBackListener != null) {
            gsyMediaBackListener.onError(what, extra);
        }
        if (mNetChanged) {
            mNetChanged = false;
            netWorkErrorLogic();
    ...
    public void clearCurrentCache() {
        if (getGSYVideoManager().isCacheFile()) {
            //是否为缓存文件
            Debuger.printfError("Play Error " + mUrl);
            mUrl = mOriginUrl;
            getGSYVideoManager().clearCache(mContext, mOriginUrl);
     ...
     * @param soundTouch
     */
    public void setSpeedPlaying(boolean soundTouch) {
        getGSYVideoManager().setSpeedPlaying(soundTouch);
    }
