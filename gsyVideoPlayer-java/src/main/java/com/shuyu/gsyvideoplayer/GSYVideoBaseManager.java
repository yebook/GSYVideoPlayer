...
public abstract class GSYVideoBaseManager implements IMediaPlayer.OnPreparedListener, IMediaPlayer.OnCompletionListener,
    ...
    public void prepare(String url, Map<String, String> mapHeadData, boolean loop) {
        prepare(url, mapHeadData, loop, null);
    }

    @Override
    public void prepare(final String url, final Map<String, String> mapHeadData, boolean loop, String overrideExtension) {
        if (TextUtils.isEmpty(url)) return;
        Message msg = new Message();
        msg.what = HANDLER_PREPARE;
        GSYModel fb = new GSYModel(url, mapHeadData, loop, 1, false, null, overrideExtension);
        msg.obj = fb;
        sendMessage(msg);
        if (needTimeOutOther) {
            startTimeOutBuffer();
